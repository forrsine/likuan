...
=======
已完成工程骨架、解析前端、Thompson NFA、子集构造 DFA、Hopcroft 最小化、捕获组、状态转移表和 NFA/DFA 双模式匹配：

- C17 + CMake 项目结构
- 对外 API：`regex_compile`、`regex_compile_ex`、`regex_match`、`regex_search`、`regex_findall`、`regex_capture_count`、`regex_free`
...
- `matches[0]` 返回整体匹配，`matches[1..]` 返回括号捕获；未参与的组为 `[-1,-1]`
- DFA 模式先定位整体区间，再使用带标签 NFA 恢复捕获组，保证最小化不丢失分组信息
